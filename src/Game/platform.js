export class Platform {
  constructor(game, lowerY, upperY, type) {
    this.game = game;
    this.width = 100; // Измененная ширина
    this.height = 25; // Измененная высота
    this.type = type;
    this.x =
      Math.floor(Math.random() * (this.game.width - this.width - 0 + 1)) + 0;
    ...
    if (this.type !== "brown") {
      if (!this.game.platforms.length) {
        ...
        return (
          ...
          (Math.floor(
            Math.random() *
              (this.game.platform_gap - (this.game.platform_gap - 30) + 1)
          ) +
    ...
    for (let i = 0; i < this.game.platforms.length; i++) {
      ...
      if (
        (y1 + this.height >= iPlatform.y - margin &&
          y1 + this.height <= iPlatform.y + iPlatform.height + margin) ||
        (y1 >= iPlatform.y - margin && y1 <= iPlatform.y + iPlatform.height + margin)
      ) {
        return true;
      }
    }
